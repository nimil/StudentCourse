...
<head>
	...
	<script type="text/javascript">
		...
		function selectNum(){
			...
			//如果在js里面用el表达式，需要加上""
			window.location.href = "AdminServlet?operation=selectlikestudent&num="+val+"&sname=${sname}&clazz=${clazzno }" ;
									...
									<td>
										<select name="clazz">
											...
											<c:forEach items="${clazzList }" var="ceach">
												<c:if test="${clazzno == ceach.clazzno }">
													<option value="${ceach.clazzno }" selected="selected">${ceach.cname }</option>
												</c:if>
												<c:if test="${clazzno != ceach.clazzno }">
								...
								<tr>
									...
									<td width="49%" align="right">
										...
										<c:if test="${studentListPage.num != 1 }">
											<a href="AdminServlet?operation=selectlikestudent&num=1&sname=${sname}&clazz=${clazzno }" class="right-font08">首页</a>
										   |<a href="AdminServlet?operation=selectlikestudent&num=${studentListPage.num-1 }&sname=${sname}&clazz=${clazzno }" class="right-font08">上一页</a> | 
										</c:if>
										
										<c:if test="${studentListPage.num != studentListPage.count }">	
											<a href="AdminServlet?operation=selectlikestudent&num=${studentListPage.num+1 }&sname=${sname}&clazz=${clazzno }" class="right-font08">下一页</a> |						
											<a href="AdminServlet?operation=selectlikestudent&num=${studentListPage.count }&sname=${sname}&clazz=${clazzno }" class="right-font08">末页</a>
										</c:if>
										] 转至：
									</td>
									<td width="1%">
